module MagicLinks
  class << self
    ...
    def magic_link_for(user, template_name, path, expiry = nil)
      template = MagicLinks.templates[template_name]
      fail ArgumentError.new('Template not found') unless template.present?
    ...
    def included(other)
      return unless other.respond_to?(:helper_method)
      other.send(:helper_method, :magic_link_for)
    end
  ...
  def magic_link_for(user, template_name, path, expiry = nil)
    MagicLinks.magic_link_for(user, template_name, path, expiry)
  end

  def magic_url_for(user, template_name, path, expiry = nil)
    MagicLinks.magic_url_for(user, template_name, path, expiry)
  end


  Template = Struct.new(:name, :pattern, :action_scope, :strength, :expiry) do
    VALID_PATTERN = %r<^/([A-Za-z0-9_\-]+)/(:token)$>
    ...
    def pattern=(val)
      fail ArgumentError.new("Pattern must be of the form '/xyz/:token'") unless VALID_PATTERN.match?(val)
    end
    ...
    def build_matcher
      sections = VALID_PATTERN.match(pattern).captures
