...
describe MagicLinks do
  ...
  let(:strength) { [:mild, :moderate, :strong].sample }
  ...
  describe 'adding a template' do
    subject { MagicLinks.tap { MagicLinks.add_template params } }
    ...
    context 'when an invalid pattern is given' do
      ...
      it 'fails' do
        expect { MagicLinks.add_template params }.to raise_error ArgumentError
      end
    end
  end
  ...
  describe '#magic_link_for' do
    ...
    let(:instance) do
      klass = Class.new
      klass.include MagicLinks
      klass.new
    end

    subject do
      MagicLinks.add_template params
