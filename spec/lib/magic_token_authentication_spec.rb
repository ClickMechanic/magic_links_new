...
describe MagicTokenAuthentication do
  let(:action_scope) { {
    bookings: [:show, :edit, :update],
    'bookings/availability': :show,
  }}
  let(:controller) { 'bookings' }
  let(:action) { 'show' }
  let(:magic_token) { create(:magic_token, :for_user, action_scope: action_scope) }
  let(:request) do
  ...
  subject { MagicTokenAuthentication.new(env, magic_token.scope) }
  ...
  describe '#clean_up_csrf?' do
    it 'is false' do
      expect(subject.clean_up_csrf?).to be_falsey
    end
  end
end
