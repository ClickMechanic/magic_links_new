...
describe MagicTokenRedirect do
  ...
  subject { MagicTokenRedirect.new(app) }

  context 'when the magic token is present' do
    # Our middleware depends on ActionDispatch::Cookies middleware.
    ...
    context 'when there is no authenticatable target' do
    ...
    context 'when there is an authenticatable target' do
      let(:magic_token) { create :magic_token, :for_user }
  ...
  describe MagicTokenRedirect::Handler do
    let(:env) {
      {
        'PATH_INFO' => path
      }
    }
    let(:handler) { MagicTokenRedirect::Handler.new(env) }
